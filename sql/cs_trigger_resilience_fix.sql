...
-- ═══════════════════════════════════════════════════════════════════
--  14c. AUTO-PROMOTION: auto_handle_absence RPC + Trigger
--       When a starter sets availability to 'no' and the lineup is
--       published, auto-promote the next reserve and notify.
-- ═══════════════════════════════════════════════════════════════════

-- Ensure cs_lineup_events table exists (audit trail for auto-promotion)
...
-- Called when a starter cancels (availability 'no') on a published lineup.
-- Promotes the next (unlocked) reserve to the vacated starter position.
-- If no reserve is available, creates a 'no_reserve' event instead.
-- Returns JSON: { promoted: true/false, ... }
...
BEGIN
  ...
  -- 2. Find the absent player's STARTER slot (skip if not a starter)
  ...
  IF v_reserve_slot IS NULL THEN
    ...
    -- Resolve name and insert no_reserve event
  ...
  -- Remove the absent player's slot (or move to last reserve)
...
-- ── Trigger: auto-promote on availability 'no' ──────────────────
-- Fires when cs_match_availability is inserted/updated with status='no'.
-- Calls auto_handle_absence if the match has a published lineup.
-- IMPORTANT: The ENTIRE body is wrapped in EXCEPTION so that this
--            trigger can NEVER block an availability change.
...
DECLARE
  v_result jsonb;
BEGIN
  -- Top-level EXCEPTION: nothing inside can ever block the availability upsert
  BEGIN
    -- Only fire when status changes TO 'no'
    IF NEW.status IS DISTINCT FROM 'no' THEN
      RETURN NEW;
    END IF;

    -- Skip if this is an UPDATE where status was already 'no'
    ...
    END IF;
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'fn_auto_promote_on_absence failed: % (SQLSTATE %)', SQLERRM, SQLSTATE;
  END;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'auto_handle_absence') THEN
    RAISE WARNING 'auto_handle_absence RPC NOT found!';
  ELSE
    RAISE NOTICE 'OK: auto_handle_absence() exists';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'fn_auto_promote_on_absence') THEN
    RAISE WARNING 'fn_auto_promote_on_absence trigger fn NOT found!';
  ELSE
    RAISE NOTICE 'OK: fn_auto_promote_on_absence() exists';
  END IF;
